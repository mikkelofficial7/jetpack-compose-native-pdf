...
package com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.encryption;
...
public abstract class SecurityHandler
{
     ...
     * @param documentIDArray  document id which is returned via {@link com.jetpack.compose.pdfDecryptor.pdfbox.cos.COSDocument#getDocumentID()}
    ...
    private void encryptDataAES256(InputStream data, OutputStream output, boolean decrypt) throws IOException
    {
        ...
        catch (IOException exception)
        {
            ...
            Log.d("Pdf-Decryptor", "A GeneralSecurityException occurred when decrypting some stream data", exception);
        }
        finally
        {
            cis.close();
    ...
    public void decryptStream(COSStream stream, long objNum, long genNum) throws IOException
    {
        ...
        if (COSName.METADATA.equals(type))
        {
            ...
            {
                Log.w("Pdf-Decryptor", "Metadata is not encrypted, but was expected to be");
                Log.w("Pdf-Decryptor", "Read PDF specification about EncryptMetadata (default value: true)");
                return;
            }
        }
        ...
        {
            Log.e("Pdf-Decryptor", ex.getClass().getSimpleName() + " thrown when decrypting object " +
                objNum + " " + genNum + " obj");
            throw ex;
        }
        finally
    ...
    private void decryptString(COSString string, long objNum, long genNum)
    {
        ...
        {
            Log.e("Pdf-Decryptor", "Failed to decrypt COSString of length " + string.getBytes().length +
