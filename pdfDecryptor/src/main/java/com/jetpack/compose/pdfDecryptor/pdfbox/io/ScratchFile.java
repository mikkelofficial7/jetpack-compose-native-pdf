 ...
 */
package com.jetpack.compose.pdfDecryptor.pdfbox.io;
...
public class ScratchFile implements Closeable
{
    ...
    public static ScratchFile getMainMemoryOnlyInstance()
    {
        ...
        catch (IOException ioe)
        {
            ...
            Log.e("Pdf-Decryptor", "Unexpected exception occurred creating main memory scratch file instance: " + ioe.getMessage() );
            return null;
        }
    }
            ...
            if (useScratchFile)
            {
                ...
                if ( raf == null )
                {
                    ...
                    catch (IOException e)
                    {
                        if (!file.delete())
                        {
                            Log.w("Pdf-Decryptor", "Error deleting scratch file: " + file.getAbsolutePath());
                        }
                        throw e;
                    }
                }
                ...
                if (pageCount + ENLARGE_PAGE_COUNT > pageCount)
                {
                    ...
                    {
                        Log.d("Pdf-Decryptor", "file: " + file);
                        Log.d("Pdf-Decryptor", "fileLen before: " + fileLen + ", raf length: " + raf.length() +
                    ...
                    if (PDFBoxConfig.isDebugEnabled())
                    {
                        Log.d("Pdf-Decryptor", "fileLen after1: " + fileLen + ", raf length: " + raf.length() +
                            ", file length: " + file.length());
                    }
                    if (fileLen != raf.length())
                    {
                        ...
                        Log.d("Pdf-Decryptor", "fileLen after2:  " + fileLen + ", raf length: " + raf.length() + ", file length: " + file.length());
                    }
