...
final class FontMapperImpl implements FontMapper
{
    ...
    FontMapperImpl()
    {
        ...
        try
        {
            String ttfName = "com/jetpack/compose/pdfDecryptor/pdfbox/resources/ttf/LiberationSans-Regular.ttf";
    ...
    private FontInfo getFont(FontFormat format, String postScriptName)
    {
        ...
        if (info != null && info.getFormat() == format)
        {
            ...
            {
                Log.d("Pdf-Decryptor", String.format("getFont('%s','%s') returns %s", format, postScriptName, info));
            }
            return info;
        }
        return null;
        ...
        if (cidSystemInfo != null)
        {
                ...
                collection.equals("Adobe-Japan1") || collection.equals("Adobe-Korea1"))
            {
                ...
                if (bestMatch != null)
                {
                    if (PDFBoxConfig.isDebugEnabled())
                    {
                        Log.d("Pdf-Decryptor", "Best match for '" + baseFont + "': " + bestMatch.info);
                    }
