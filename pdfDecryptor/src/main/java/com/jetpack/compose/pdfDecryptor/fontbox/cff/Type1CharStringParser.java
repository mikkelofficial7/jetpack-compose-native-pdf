    ...
    private List<Object> parse(byte[] bytes, List<byte[]> subrs, List<Object> sequence) throws IOException
    {
        ...
        while (input.hasRemaining())
        {
            ...
            if (b0 == CALLSUBR)
            {
                ...
                {
                    Log.w("Pdf-Decryptor", "Parameter " + obj + " for CALLSUBR is ignored, integer expected in glyph '"
                ...
                {
                    Log.w("Pdf-Decryptor", "CALLSUBR is ignored, operand: " + operand
            ...
            else if (b0 == TWO_BYTE && input.peekUnsignedByte(0) == CALLOTHERSUBR)
            {
                ...
                if (results.size() > 0)
                {
                    Log.w("Pdf-Decryptor", "Value left on the PostScript stack in glyph " + glyphName + " of font " + fontName);
                }
            }
            else if (b0 >= 0 && b0 <= 31)
            {
