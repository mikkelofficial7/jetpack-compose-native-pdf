...
public class AcroFormGenerateAppearancesProcessor extends AbstractProcessor
{
   ...
   public void process() {
      ...
      if (acroForm != null && acroForm.getNeedAppearances())
      {
         try
         {
            Log.d("Pdf-Decryptor", "trying to generate appearance streams for fields as NeedAppearances is true()");
            acroForm.refreshAppearances();
            acroForm.setNeedAppearances(false);
         }
         catch (IOException ioe)
         {
            Log.d("Pdf-Decryptor", "couldn't generate appearance stream for some fields - check output");
            Log.d("Pdf-Decryptor", ioe.getMessage());
         }
         catch (IllegalArgumentException iae)
         {
            Log.d("Pdf-Decryptor", "couldn't generate appearance stream for some fields - check output");
            Log.d("Pdf-Decryptor", iae.getMessage());
         }
      }
   }
} 