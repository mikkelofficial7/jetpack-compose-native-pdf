...
public abstract class BaseParser
{
    ...
    private COSBase parseCOSDictionaryValue() throws IOException
    {
        ...
        {
            Log.e("Pdf-Decryptor", "expected number, actual=" + value + " at offset " + numOffset);
            return COSNull.NULL;
        }
        if (!(generationNumber instanceof COSInteger))
        {
            Log.e("Pdf-Decryptor", "expected number, actual=" + generationNumber + " at offset " + genOffset);
            return COSNull.NULL;
        }
        long objNumber = ((COSInteger) value).longValue();
        if (objNumber <= 0)
        {
            Log.w("Pdf-Decryptor", "invalid object number value =" + objNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
        int genNumber = ((COSInteger) generationNumber).intValue();
        if (genNumber < 0)
        {
            Log.e("Pdf-Decryptor", "invalid generation number value =" + genNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
    ...
    protected COSDictionary parseCOSDictionary() throws IOException
    {
        ...
        while (!done)
        {
            ...
            else
            {
                ...
                Log.w("Pdf-Decryptor", "Invalid dictionary, found: '" + c + "' but expected: '/' at offset " + seqSource.getPosition());
    ...
    private boolean parseCOSDictionaryNameValuePair(COSDictionary obj) throws IOException
    {
        ...
        {
            Log.w("Pdf-Decryptor", "Empty COSName at offset " + seqSource.getPosition());
        }
        COSBase value = parseCOSDictionaryValue();
        skipSpaces();
        if (value == null)
        {
            Log.w("Pdf-Decryptor", "Bad dictionary declaration at offset " + seqSource.getPosition());
            return false;
        }
        else if (value instanceof COSInteger && !((COSInteger)value).isValid())
        {
            Log.w("Pdf-Decryptor", "Skipped out of range number value at offset " + seqSource.getPosition());
        }
        else
        {
    ...
    protected COSArray parseCOSArray() throws IOException
    {
        ...
        while( ((i = seqSource.peek()) > 0) && ((char)i != ']') )
        {
            ...
            else
            {
                ...
                Log.w("Pdf-Decryptor", "Corrupt array element at offset "
        ...
        while (c != -1)
        {
            ...
            if (ch == '#')
            {
                ...
                else
                {
                    ...
                    if (ch2 == -1 || ch1 == -1)
                    {
                        Log.e("Pdf-Decryptor", "Premature EOF in BaseParser#parseCOSName");
                        c = -1;
                        break;
                    }
    ...
    protected COSBase parseDirObject() throws IOException
    {
        ...
        switch(c)
        {
            ...
            default:
                ...
                {
                    Log.w("Pdf-Decryptor", "Skipped unexpected dir object = '" + badString + "' at offset "
