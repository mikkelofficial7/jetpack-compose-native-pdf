 ...
 */
package com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.form;
...
import com.jetpack.compose.pdfDecryptor.pdfbox.contentstream.operator.Operator;
...
import com.jetpack.compose.pdfDecryptor.pdfbox.cos.COSString;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdfparser.PDFStreamParser;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdfwriter.ContentStreamWriter;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.PDPageContentStream;
...
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.font.PDType3CharProc;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.font.PDType3Font;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.font.PDVectorFont;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.graphics.color.PDColor;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.action.PDAction;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.action.PDActionJavaScript;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.action.PDFormFieldAdditionalActions;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDAnnotationWidget;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDAppearanceCharacteristicsDictionary;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDAppearanceDictionary;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDAppearanceEntry;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDAppearanceStream;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.interactive.annotation.PDBorderStyleDictionary;
...
class AppearanceGeneratorHelper {
        ...
        for (PDAnnotationWidget widget : field.getWidgets())
        {
            ...
            for (COSName fontResourceName : widgetResources.getFontNames())
            {
                try
                {
                    ...
                    {
                        Log.d("Pdf-Decryptor", "Adding font resource " + fontResourceName + " from widget to AcroForm");
                ...
                {
                    Log.w("Pdf-Decryptor", "Unable to match field level font with AcroForm font");
                }
            }
        }
    }
    ...
    public void setAppearanceValue(String apValue) throws IOException {
        ...
        for (PDAnnotationWidget widget : field.getWidgets()) {
            ...
            {
                Log.w("Pdf-Decryptor", "widget of field " + field.getFullyQualifiedName() + " is a PaperMetaData widget, no appearance stream created");
                continue;
            }
            ...
            if (rect == null) {
                ...
                Log.w("Pdf-Decryptor", "widget of field " + field.getFullyQualifiedName()
    ...
    private String getFormattedValue(String apValue)
    {
        ...
        if (actionF != null)
        {
            ...
            Log.i("Pdf-Decryptor", "Field contains a formatting action but no ScriptingHandler " +
    ...
    private void insertGeneratedAppearance(PDAnnotationWidget widget, PDAppearanceStream appearanceStream,
        ...
        if (font.getName().contains("+")) {
            Log.w("Pdf-Decryptor", "Font '" + defaultAppearance.getFontName().getName() + "' of field '"
                + field.getFullyQualifiedName() + "' contains subsetted font '" + font.getName() + "'");
            Log.w("Pdf-Decryptor", "This may bring trouble with PDField.setValue(), PDAcroForm.flatten() or "
                + "PDAcroForm.refreshAppearances()");
            Log.w("Pdf-Decryptor", "You should replace this font with a non-subsetted font:");
            Log.w("Pdf-Decryptor", "PDFont font = PDType0Font.load(doc, new FileInputStream(fontfile), false);");
            Log.w("Pdf-Decryptor", "acroForm.getDefaultResources().put(COSName.getPDFName(\""
        ...
        if (font.getFontDescriptor() != null) {
            ...
            Log.d("Pdf-Decryptor", "missing font descriptor - resolved Cap/Descent to " + fontCapHeight + "/" + fontDescent);
    ...
    private float resolveGlyphHeight(PDFont font, int code) throws IOException {
        ...
        if (font instanceof PDType3Font) {
            ...
            Log.w("Pdf-Decryptor", "Unknown font class: " + font.getClass());
        }
        if (path == null) {
            return -1;
        }
