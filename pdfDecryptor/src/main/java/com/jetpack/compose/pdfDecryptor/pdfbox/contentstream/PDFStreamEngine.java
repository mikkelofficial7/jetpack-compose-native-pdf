...
import com.jetpack.compose.pdfDecryptor.pdfbox.contentstream.operator.MissingOperandException;
import com.jetpack.compose.pdfDecryptor.pdfbox.contentstream.operator.Operator;
import com.jetpack.compose.pdfDecryptor.pdfbox.contentstream.operator.OperatorProcessor;
import com.jetpack.compose.pdfDecryptor.pdfbox.contentstream.operator.state.EmptyGraphicsStackException;
...
import com.jetpack.compose.pdfDecryptor.pdfbox.filter.MissingImageReaderException;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdfparser.PDFStreamParser;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.MissingResourceException;
...
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.graphics.pattern.PDTilingPattern;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.graphics.state.PDGraphicsState;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.graphics.state.PDTextState;
...
public abstract class PDFStreamEngine
{
    ...
    public void showTextStrings(COSArray array) throws IOException
    {
        ...
        for (COSBase obj : array)
        {
            ...
            {
                Log.e("Pdf-Decryptor", "Nested arrays are not allowed in an array for TJ operation: " + obj);
            }
            else
            {
                Log.e("Pdf-Decryptor", "Unknown type " + obj.getClass().getSimpleName()
    ...
    protected void showText(byte[] string) throws IOException
    {
        ...
        if (font == null)
        {
            Log.w("Pdf-Decryptor", "No current font, will use default");
        ...
        throws IOException
    {
        ...
        {
            Log.e("Pdf-Decryptor", e.getMessage());
        }
        else if (e instanceof EmptyGraphicsStackException)
        {
            Log.w("Pdf-Decryptor", e.getMessage());
        }
    ...
    public void setLineDashPattern(COSArray array, int phase)
    {
        if (phase < 0)
        {
            Log.w("Pdf-Decryptor", "Dash phase has negative value " + phase + ", set to 0");
            phase = 0;
        }
    ...
    public void decreaseLevel()
    {
        ...
        if (level < 0)
        {
            Log.e("Pdf-Decryptor", "level is " + level);
        }
    }
}
