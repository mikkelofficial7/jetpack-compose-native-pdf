...
public final class StandardSecurityHandler extends SecurityHandler
{
            ...
            if (stdCryptFilterDictionary != null)
            {
                ...
                if (COSName.AESV2.equals(cryptFilterMethod))
                {
                    ...
                    if (encryption.getCOSObject().containsKey(COSName.LENGTH))
                    {
                        ...
                        {
                            Log.w("Pdf-Decryptor", "Using " + newLength + " bytes key length instead of " +
    ...
    private void validatePerms(PDEncryption encryption, int dicPermissions, boolean encryptMetadata) throws IOException
    {
        try
        {
            ...
            {
                Log.w("Pdf-Decryptor", "Verification of permissions failed (constant)");
            }
            ...
            {
                Log.w("Pdf-Decryptor", "Verification of permissions failed (" + String.format("%08X",permsP) +
                    " != " + String.format("%08X",dicPermissions) + ")");
            }

            if (encryptMetadata && perms[8] != 'T' || !encryptMetadata && perms[8] != 'F')
            {
                Log.w("Pdf-Decryptor", "Verification of permissions failed (EncryptMetadata)");
            }
        }
    ...
    private static void logIfStrongEncryptionMissing()
    {
        try
        {
            ...
            {
                Log.w("Pdf-Decryptor", "JCE unlimited strength jurisdiction policy files are not installed");
            }
        }
