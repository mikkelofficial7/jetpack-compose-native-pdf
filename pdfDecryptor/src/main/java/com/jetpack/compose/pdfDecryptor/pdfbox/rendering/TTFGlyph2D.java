...
final class TTFGlyph2D implements Glyph2D
{
    ...
    public Path getPathForGID(int gid, int code) throws IOException
    {
        if (gid == 0 && !isCIDFont && code == 10 && font.isStandard14())
        {
            ...
            Log.w("Pdf-Decryptor", "No glyph for code " + code + " in font " + font.getName());
            return new Path();
        }
        ...
        if (glyphPath == null)
        {
            if (gid == 0 || gid >= ttf.getMaximumProfile().getNumGlyphs())
            {
                if (isCIDFont)
                {
                    ...
                    Log.w("Pdf-Decryptor", "No glyph for code " + code + " (CID " + cidHex + ") in font " +
                        font.getName());
                }
                else
                {
                    Log.w("Pdf-Decryptor", "No glyph for " + code + " in font " + font.getName());
                }
            }
