...
import com.jetpack.compose.pdfDecryptor.fontbox.cff.Type2CharString;
import com.jetpack.compose.pdfDecryptor.fontbox.cmap.CMap;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.CmapLookup;
...
public class PDCIDFontType2 extends PDCIDFont
{
            ...
            if (stream != null)
            {
                try
                {
                    ...
                    if (otf.isPostScript())
                    {
                        ...
                        fontIsDamaged = true;
                        Log.w("Pdf-Decryptor", "Found CFF/OTF but expected embedded TTF font " + fd.getFontName());
                    }
                }
                catch (IOException e)
                {
                    fontIsDamaged = true;
                    Log.w("Pdf-Decryptor", "Could not read embedded OTF for font " + getBaseFont(), e);
                }
            }
    ...
    private TrueTypeFont findFontOrSubstitute() throws IOException
    {
        ...
        {
            Log.w("Pdf-Decryptor", "Using fallback font " + ttfFont.getName() + " for CID-keyed TrueType font " + getBaseFont());
        }
        return ttfFont;
    }
    ...
    public int codeToGID(int code) throws IOException
    {
        if (!isEmbedded)
        {
            ...
            if (cid2gid != null && !isDamaged)
            {
                ...
                Log.w("Pdf-Decryptor", "Using non-embedded GIDs in font " + getName());
            ...
            else
            {
                ...
                if (unicode == null)
                {
                    if (!noMapping.contains(code))
                    {
                        ...
                        noMapping.add(code);
                        Log.w("Pdf-Decryptor", "Failed to find a character mapping for " + code + " in " + getName());
                    }
                ...
                {
                    Log.w("Pdf-Decryptor", "Trying to map multi-byte character using 'cmap', result will be poor");
                }
