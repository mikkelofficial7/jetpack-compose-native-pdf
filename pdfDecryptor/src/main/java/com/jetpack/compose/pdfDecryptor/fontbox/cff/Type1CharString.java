 ...
 */
package com.jetpack.compose.pdfDecryptor.fontbox.cff;
...
import com.jetpack.compose.pdfDecryptor.fontbox.encoding.StandardEncoding;
import com.jetpack.compose.pdfDecryptor.fontbox.type1.Type1CharStringReader;
...
public class Type1CharString
{
    ...
    private List<Number> handleCommand(List<Number> numbers, CharStringCommand command)
    {
        ...
        else if ("return".equals(name) || "callsubr".equals(name))
        {
            ...
            Log.w("Pdf-Decryptor", "Unexpected charstring command: " + name + " in glyph " +
        ...
        {
            // indicates an invalid charstring
            Log.w("Pdf-Decryptor", "Unknown charstring command: " + command.getKey() + " in glyph " + glyphName +
                " of font " + fontName);
        }
        return null;
    }
    ...
    private void callothersubr(int num)
    {
        if (num == 0)
        {
            ...
            {
                Log.w("Pdf-Decryptor", "flex without moveTo in font " + fontName + ", glyph " + glyphName +
        ...
        {
            Log.w("Pdf-Decryptor", "Invalid callothersubr parameter: " + num);
        }
    }
    ...
    private void rlineTo(Number dx, Number dy)
    {
        ...
        if (path.isEmpty())
        {
            Log.w("Pdf-Decryptor", "rlineTo without initial moveTo in font " + fontName + ", glyph " + glyphName);
            path.moveTo(x, y);
        }
        else
        {
        ...
        Number dx3, Number dy3)
    {
        ...
        if (path.isEmpty())
        {
            Log.w("Pdf-Decryptor", "rrcurveTo without initial moveTo in font " + fontName + ", glyph " + glyphName);
            path.moveTo(x3, y3);
        }
        else
        {
    ...
    private void closeCharString1Path()
    {
        if (path.isEmpty())
        {
            Log.w("Pdf-Decryptor", "closepath without initial moveTo in font " + fontName + ", glyph " + glyphName);
        }
        else
        {
            path.close();
    ...
    private void seac(Number asb, Number adx, Number ady, Number bchar, Number achar)
    {
        ...
        {
            Log.w("Pdf-Decryptor", "invalid seac character in glyph " + glyphName + " of font " + fontName);
        }
        // accent character
        ...
        try
        {
            ...
            if (path == accent.getPath())
            {
                ...
                Log.w("Pdf-Decryptor", "Path for " + baseName + " and for accent " + accentName + " are same, ignored");
                return;
            }
