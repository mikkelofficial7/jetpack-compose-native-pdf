 ...
 */
package com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.font;
...
import com.jetpack.compose.pdfDecryptor.fontbox.FontBoxFont;
import com.jetpack.compose.pdfDecryptor.fontbox.cff.CFFCIDFont;
import com.jetpack.compose.pdfDecryptor.fontbox.cff.CFFFont;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.NamingTable;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.OS2WindowsMetricsTable;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.OTFParser;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.OpenTypeFont;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.TTFParser;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.TrueTypeCollection;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.TrueTypeCollection.TrueTypeFontProcessor;
import com.jetpack.compose.pdfDecryptor.fontbox.ttf.TrueTypeFont;
import com.jetpack.compose.pdfDecryptor.fontbox.type1.Type1Font;
import com.jetpack.compose.pdfDecryptor.fontbox.util.autodetect.FontFileFinder;
import com.jetpack.compose.pdfDecryptor.pdfbox.android.PDFBoxConfig;
    ...
    private static class FSFontInfo extends FontInfo
    {
        ...
        private TrueTypeFont getTrueTypeFont(String postScriptName, File file)
        {
            try
            {
                ...
                {
                    Log.d("Pdf-Decryptor", "Loaded " + postScriptName + " from " + file);
                }
                return ttf;
            }
            catch (IOException e)
            {
                Log.w("Pdf-Decryptor", "Could not load font file: " + file, e);
            }
            return null;
        }
        ...
        private OpenTypeFont getOTFFont(String postScriptName, File file)
        {
            try
            {
                if (file.getName().toLowerCase().endsWith(".ttc"))
                {
                    ...
                    {
                        Log.e("Pdf-Decryptor", ex.getMessage(), ex);
                        ttc.close();
                        return null;
                    }
                    if (ttf == null)
...
//                addTrueTypeFont(new File("/system/fonts/DroidSansFallback.ttf"));
        ...
        try
        {
            if (PDFBoxConfig.isDebugEnabled())
            {
                Log.d("Pdf-Decryptor", "Will search the local system for fonts");
            }
            ...
            if (PDFBoxConfig.isDebugEnabled())
            {
                Log.d("Pdf-Decryptor", "Found " + files.size() + " fonts on the local system");
            }
            ...
            if (!files.isEmpty())
            {
                ...
                {
                    Log.w("Pdf-Decryptor", "Building on-disk font cache, this may take a while");
                    scanFonts(files);
                    saveDiskCache();
                    Log.w("Pdf-Decryptor", "Finished building on-disk font cache, found " + fontInfoList.size()
                        + " fonts");
                }
            }
        }
        ...
        {
            Log.e("Pdf-Decryptor", "Error accessing the file system", e);
        }
    }
    ...
    private void scanFonts(List<File> files)
    {
        for (File file : files)
        {
            ...
            {
                Log.w("Pdf-Decryptor", "Error parsing font " + file.getPath(), e);
            }
        }
    }
    ...
    private void saveDiskCache()
    {
        ...
        {
            Log.w("Pdf-Decryptor", "Could not write to font cache", e);
            Log.w("Pdf-Decryptor", "Installed fonts information will have to be reloaded for each start");
            Log.w("Pdf-Decryptor", "You can assign a directory to the 'pdfbox.fontcache' property");
        }
        finally
        {
    ...
    private List<FSFontInfo> loadDiskCache(List<File> files)
    {
        ...
        if (fileExists)
        {
            ...
            try
            {
                ...
                while ((line = reader.readLine()) != null)
                {
                    ...
                    if (parts.length < 10)
                    {
                        Log.w("Pdf-Decryptor", "Incorrect line '" + line + "' in font disk cache is skipped");
                        continue;
                    }
                    ...
                    {
                        Log.d("Pdf-Decryptor", "Font file " + fontFile.getAbsolutePath() + " not found, skipped");
                    }
            ...
            {
                Log.w("Pdf-Decryptor", "Error loading font cache, will be re-built", e);
                return null;
            }
            finally
            {
        ...
        if (!pending.isEmpty())
        {
            ...
            Log.w("Pdf-Decryptor", "New fonts found, font cache will be re-built");
            return null;
        }
    ...
    private void addTrueTypeCollection(final File ttcFile) throws IOException
    {
        ...
        {
            Log.w("Pdf-Decryptor", "Could not load font file: " + ttcFile, e);
        }
        finally
        {
            if (ttc != null)
    ...
    private void addTrueTypeFont(File ttfFile) throws IOException
    {
        ...
        {
            Log.w("Pdf-Decryptor", "Could not load font file: " + ttfFile, e);
        }
    }
    ...
    private void addTrueTypeFontImpl(TrueTypeFont ttf, File file) throws IOException
    {
        try
        {
            ...
            if (ttf.getName() != null && ttf.getName().contains("|"))
            {
                ...
                Log.w("Pdf-Decryptor", "Skipping font with '|' in name " + ttf.getName() + " in file " + file);
            }
            else if (ttf.getName() != null)
            {
                ...
                if (PDFBoxConfig.isDebugEnabled())
                {
                    ...
                    if (name != null)
                    {
                        Log.d("Pdf-Decryptor", format +": '" + name.getPostScriptName() + "' / '" +
                            name.getFontFamily() + "' / '" +
            ...
            else
            {
                ...
                Log.w("Pdf-Decryptor", "Missing 'name' entry for PostScript name in font " + file);
            }
        }
        catch (IOException e)
        {
    ...
    private void addType1Font(File pfbFile) throws IOException
    {
        ...
        try
        {
            ...
            if (type1.getName() == null)
            {
                ...
                Log.w("Pdf-Decryptor", "Missing 'name' entry for PostScript name in font " + pfbFile);
                return;
            }
            if (type1.getName().contains("|"))
            {
                fontInfoList.add(new FSIgnored(pfbFile, FontFormat.PFB, "*skippipeinname*"));
                Log.w("Pdf-Decryptor", "Skipping font with '|' in name " + type1.getName() + " in file " + pfbFile);
                return;
            }
            ...
            if (PDFBoxConfig.isDebugEnabled())
            {
                Log.d("Pdf-Decryptor", "PFB: '" + type1.getName() + "' / '" + type1.getFamilyName() + "' / '" +
                    type1.getWeight() + "'");
            }
        }
        catch (IOException e)
        {
            Log.w("Pdf-Decryptor", "Could not load font file: " + pfbFile, e);
        }
        finally
        {
            input.close();
