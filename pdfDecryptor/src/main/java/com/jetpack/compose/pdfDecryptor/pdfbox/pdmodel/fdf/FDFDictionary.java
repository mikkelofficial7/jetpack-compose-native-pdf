...
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.common.filespecification.PDFileSpecification;
import com.jetpack.compose.pdfDecryptor.pdfbox.pdmodel.common.filespecification.PDSimpleFileSpecification;
        ...
        for (int i = 0; i < nodeList.getLength(); i++)
        {
            ...
            if (node instanceof Element)
            {
                ...
                else if (child.getTagName().equals("ids"))
                {
                    ...
                    {
                        Log.w("Pdf-Decryptor", "Error parsing ID entry for attribute 'original' [" + original
                            + "]. ID entry ignored.", e);
                    }
                    try
                    {
                    ...
                    {
                        Log.w("Pdf-Decryptor", "Error parsing ID entry for attribute 'modified' [" + modified
                            + "]. ID entry ignored.", e);
                    }
                    setID(ids);
                }
                else if (child.getTagName().equals("fields"))
                {
                    ...
                    for (int f = 0; f < fields.getLength(); f++)
                    {
                            ...
                            && ((Element) currentNode).getTagName().equals("field"))
                        {
                            ...
                            {
                                Log.w("Pdf-Decryptor", "Error parsing field entry [" + currentNode.getNodeValue()
                                    + "]. Field ignored.", e);
                            }
                        }
                    }
